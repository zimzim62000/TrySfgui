...
void PathFinding::findRoad(std::shared_ptr<MapGame> map, const int start_x, const int start_y, const int end_x, const int end_y)
{
...
void PathFinding::ajouter_cases_adjacentes(std::pair <int, int>& n, std::shared_ptr<MapGame> map) {
	noeud tmp;
	...
	for (int i = n.first - 1; i <= n.first + 1; i++) {
		...
		for (int j = n.second - 1; j <= n.second + 1; j++) {
			...
			if ((i == n.first) && (j == n.second))  // case actuelle n
				continue;
			if (map->getAtThisPositionNoeud(i, j)->passable == false)
			...
			if (((i == n.first - 1 || i == n.first + 1) && j == n.second) || ((j == n.second - 1 || j == n.second + 1) && i == n.first))
			{
				...
				if (!this->deja_present_dans_liste(it, this->liste_fermee)) {
					...
					tmp.cout_g = this->liste_fermee[n].cout_g + this->distance(i, j, n.first, n.second) + map->getAtThisPositionNoeud(i, j)->weight;
